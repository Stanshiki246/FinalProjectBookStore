...
def delete_product(id):
    ...
    if product is not None:
        cart_product=Cart_products.query.filter_by(user_id=id).first_or_404()
        if cart_product is not None:
            db.session.delete(cart_product)
            db.session.commit()
        trans_product=Trans_products.query.filter_by(user_id=id).first_or_404()
        if trans_product is not None:
            db.session.delete(trans_product)
            db.session.commit()
        library_product=Library_products.query.filter_by(user_id=id).first_or_404()
...
def product(id):
    ...
    return render_template('ProductDetails.html',title='Product Details',product=Products.query.filter_by(id=id).first_or_404(),
                           comic=Comics.query.filter_by(product_id=id).first_or_404(),
...
def manage_payment():
    ...
    return render_template('ManagePayment.html',title='Manage Payment',payments=res.json()['Results'])
...
def accept_payment(id):
    res = requests.delete('http://127.0.0.1:5000/payments/',params={'order_id': id})
